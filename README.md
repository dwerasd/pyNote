...
- **파일로 열고 저장(메모장 대체)**: `열기…`(Ctrl+O) 또는 `pyNote.exe 파일경로`
  (탐색기 더블클릭)로 연 파일은 카드 한 장에 결속되고, Ctrl+S 가 리비전 확정과
  함께 원래 인코딩·BOM·줄바꿈 그대로 그 파일에 되쓴다. `다른 이름으로 저장…`
  (Ctrl+Shift+S)·`파일 연결 끊기` 제공, 바깥에서 바뀐 파일은 덮어쓰기 전에 묻는다.
  DB 가 정본이고 파일은 거울이다 — 가져오기·드롭·내보내기는 종전대로 사본만 만든다
...
- **문서 관리**: 문서·카드 검색(`문서 목록…` 은 Ctrl+Shift+O), 보관함과 휴지통,
  재시작 시 작업 상태 복원
