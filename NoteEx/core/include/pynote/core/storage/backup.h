...
namespace pynote::core::storage
{
	...
	enum class E_BACKUP_RESULT
	{
		...
		TargetInvalid,     // 복원 대상 DB 세트 경로가 일반 파일이 아니다. 원본 BackupError 자리다(:493).
		Cancelled,         // 분할 백업 루프가 취소됐다. 아래 개선이 들여온 유일한 새 경로다.
		RollbackFailed,    // 복원 실패 뒤 원본 세트 롤백까지 실패했다(:189~191). 원래 실패와 구별한다.
	...
	//  2. 복원은 본체와 -wal, -shm 을 한 세트로 다룬다(:479~484). 존재하지만 일반 파일이 아니거나
	//     심볼릭 링크인 대상은 아무것도 옮기기 전에 거절한다(:487~493).
	//  3. 복원은 기존 세트를 임시 이름으로 비켜 두고 새 본체를 제자리에 놓는다. 어느 단계든
	//     실패하면 옮긴 순서의 역순으로 전부 되돌리고, 되돌리지 못한 경로를 보고한다(:496~516).
	//     롤백 자체의 실패는 원래 실패와 다른 오류다(:184~191).
	...
	// 사전 마이그레이션 백업 훅. 파이썬 원본 MigrationBackupHook(:202) 이식이다.
	...
	{
	public:
		// 원본의 판정은 `self._backup_directory or database_path.parent / "backups"`(:222) 이고
		...
		// 원본 __call__(:215~229). 실패하면 false 이며 러너가 마이그레이션을 진행하지 않는다.
		bool operator()(const std::string& _sDatabasePath, int _nCurrentVersion, int _nLatestVersion);

		// 원본 last_backup_path(:213). 아직 만든 적이 없으면 빈 문자열이다.
	...
	// 설정된 시간 간격에 따라 자동 백업 실행 여부를 정한다. 원본 AutomaticBackupManager(:232) 이식이다.
	...
	public:
		...
		// 원본 set_interval_hours(:249~253). 0 이하는 거절한다 - 원본이 ValueError 를 올리는 자리다.
		...
		// 원본 run_if_due(:255~267). 주기가 지나지 않았으면 Ok 이면서 _pbCreated 가 false 다
	...
	private:
		// 원본 _latest_backup_time(:269~275). 후보가 없으면 false 다.
	...
	// 원본 PeriodicQuickCheck(:278) 의 세 갈래다. 원본은 "실행했는가"를 bool 로 돌려주고
	...
	// 호출 시점에 주기가 도래했으면 quick_check 를 수행한다. 원본 PeriodicQuickCheck(:278) 이식이다.
	class C_PERIODIC_QUICK_CHECK
	{
	public:
		C_PERIODIC_QUICK_CHECK(sqlite3* _pConnection, double _dIntervalHours, MonotonicSecFn _fnClock);

		// 원본 __init__ 은 0 이하 주기에 ValueError 를 올린다(:288~289). 생성자가 실패를 알릴
